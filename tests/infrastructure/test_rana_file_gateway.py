...
from rana_sdk.domain import FileStat, FileUpload, History
from rana_sdk.infrastructure import RanaApiProvider, RanaFileGateway
...
def test_stat(gateway: RanaFileGateway, provider: Mock):
    ...
    provider.job_request.assert_called_once_with("GET", "files/stat", params={"path": "path", "ref": "main"})
...
def test_upload_start(gateway: RanaFileGateway, provider: Mock):
    ...
    provider.job_request.assert_called_once_with("POST", "files/upload", params={"path": "path"})
...
def test_upload_complete(gateway: RanaFileGateway, provider: Mock, kwargs: dict, expected: dict):
    provider.job_request.return_value = {
        "id": "path",
        "last_modified": "2021-01-01T00:00:00Z",
        "url": None,
    ...
    provider.job_request.assert_called_once_with("PUT", "files/upload", json={"foo": "bar", **expected})


def test_history(gateway: RanaFileGateway, provider: Mock):
    history = History.model_validate({"ref": "abc123", "created_at": "2021-01-01T00:00:00Z"})
    provider.job_request.return_value = {"next": None, "limit": 10, "items": [history.model_dump()]}
