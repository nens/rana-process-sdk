...
class TestDirectoryOutputWithExpectedFiles(RanaProcessParameters):
    x: Annotated[
        ...
        UsingWidget(
            DirectoryPickerWidget(
                expected_files={
                    "elevation.tif": ExpectedFile(data_type="raster", meta={"physical_quantity": "digital_elevation"}),
                }
            )
        ),
    ]
...
def threedi_api_key_gateway() -> Iterator[Mock]:
    with patch.object(RanaContext, "_threedi_api_key_gateway", new_callable=PropertyMock(ThreediApiKeyGateway)) as m:
        yield m
...
def rana_schematisation_gateway() -> Iterator[Mock]:
    with patch.object(
        RanaContext, "_rana_schematisation_gateway", new_callable=PropertyMock(RanaSchematisationGateway)
    ) as m:
        yield m
...
def rana_dataset_gateway() -> Iterator[Mock]:
    with patch.object(RanaContext, "_rana_dataset_gateway", new_callable=PropertyMock(RanaDatasetGateway)) as m:
        yield m
...
def lizard_raster_layer_gateway() -> Iterator[Mock]:
    with patch.object(
        RanaContext, "lizard_raster_layer_gateway", new_callable=PropertyMock(LizardRasterLayerGateway)
    ) as m:
        yield m
...
def test_base_rana_context_set_output(rana_runtime: Mock, base_rana_context: RanaContext[Output]):
    base_rana_context.set_output(Output(number=3))

    rana_runtime.set_result.assert_called_once_with({"number": 3})


def test_base_rana_context_set_output_with_dict(rana_runtime: Mock, base_rana_context: RanaContext[Output]):
    base_rana_context.set_output({"number": 3})

    rana_runtime.set_result.assert_called_once_with({"number": 3})


def test_base_rana_context_set_output_bad_dict(rana_runtime: Mock, base_rana_context: RanaContext[Output]):
...
@patch.object(RanaContext, "upload", return_value=RanaPath(id="a/foo.txt", ref="abc123"))
...
def test_base_rana_context_set_output_with_file_ignored(upload: Mock, rana_runtime: Mock):
...
@patch.object(RanaContext, "upload_schematisation", return_value=RanaPath(id="a/foo.txt", ref="abc123"))
...
@patch.object(RanaContext, "upload", return_value=RanaPath(id="a/foo.tiff", ref="abc123"))
def test_base_rana_context_set_output_with_raster(
    upload: Mock, raster_base_rana_context: RanaContext[RasterOutput], rana_runtime: Mock, tmp_path: Path
):
...
def test_base_rana_context_set_output_with_raster_override_meta(
    ...
    path = tmp_path / "local.tiff"
    open(path, "a").close()
    raster_base_rana_context.set_output({"x": path}, {"x": {"physical_quantity": "digital_elevation"}})

    # the data_type should be set to "raster"
    upload.assert_called_once_with(
        path, "a/foo.tiff", data_type="raster", meta={"physical_quantity": "digital_elevation"}
    )
    rana_runtime.set_result.assert_called_once()
...
@mark.parametrize(
    ...
    [
        ...
        (
            ...
            {
                "x": FileOutput(
                    is_optional=False, data_type="vector", meta_values={"feature_type_definition": "study_area"}
                )
            },
        ),
        (
                ...
                "x": DirectoryOutput(
                    ...
                    expected_files={
                        "elevation.tif": FileOutput(
                            is_optional=True, data_type="raster", meta_values={"physical_quantity": "digital_elevation"}
                        )
                    },
                ),
            },
...
def test_base_rana_context_expected_output_paths(model: type[BaseModel], expected: dict[str, TestFileOutput]):
...
def test_expected_files():
    path_picker = DirectoryPickerWidget(
        expected_files={
            "elevation.tif": ExpectedFile(data_type="raster", meta={"physical_quantity": "digital_elevation"}),
            "study_area.gpkg": ExpectedFile(data_type="vector", meta={"feature_type_definition": ["study_area"]}),
    ...
    assert actual == {
        ...
        "study_area.gpkg": FileOutput(
            is_optional=True, data_type="vector", meta_values={"feature_type_definition": ["study_area"]}
        ),
...
def test_upload_schematisation(
    ...
    actual = schematisation_rana_context.upload_schematisation(schematisation_id, rana_path)

    assert actual == RanaPath(id=rana_path, ref="main")
    rana_runtime.logger.info.assert_called_once_with(f"Writing schematisation to '{rana_path}'...")
    rana_schematisation_gateway.upload.assert_called_once_with(rana_path, schematisation_id)


@patch.object(RanaContext, "upload_dir", return_value=RanaPath(id="a/foo/", ref="abc123"))
def test_base_rana_context_set_output_directory(upload_dir: Mock, rana_runtime: Mock):
    file_base_rana_context = RanaContext[TestDirectoryOutput](output_paths={"x": "a/foo/"})
...
@patch.object(RanaContext, "upload_dir", return_value=RanaPath(id="a/foo/", ref="abc123"))
def test_base_rana_context_set_output_directory_with_expected_files(upload_dir: Mock, rana_runtime: Mock):
    file_base_rana_context = RanaContext[TestDirectoryOutputWithExpectedFiles](output_paths={"x": "a/foo/"})
...
@patch.object(RanaContext, "upload")
def test_base_rana_context_upload_dir(upload: Mock, job_working_dir: Path, base_rana_context: RanaContext):
    ...
    upload.assert_has_calls(
        [
            call(local_path=local_path / "foo.txt", rana_path="a/foo.txt", data_type=None, meta={}),
            call(local_path=local_path / "foo" / "bar.txt", rana_path="a/foo/bar.txt", data_type=None, meta={}),
        ]
    )
...
def test_log_exception_process_user_error(base_rana_context: RanaContext, rana_runtime: Mock):
...
def test_log_exception_process_internal_error(base_rana_context: RanaContext, rana_runtime: Mock):
...
def test_get_lizard_raster(base_rana_context: RanaContext, lizard_raster_layer_gateway: Mock):
