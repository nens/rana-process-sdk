...
def test_using_widget():
    ...
    assert TypeAdapter(SomeType).json_schema() == {"type": "string", "rana_widget": {"id": "some_id"}}
...
def test_using_widget_override():
    ...
    assert TypeAdapter(SomeDerivedType).json_schema() == {"type": "string", "rana_widget": {"id": "some_other_id"}}
...
def test_using_widget_in_model_field(widget: Widget):
    ...
    assert SomeModel.model_json_schema()["properties"]["a"]["rana_widget"] == {"id": widget.id}
